...
Application::Application(AppConfig config, bool gl) {
  ...
  renderer = new RaytracedRenderer (
    ...
    config.pathtracer_use_ambient_gi,
    config.pathtracer_visualize_cache
  );
    ...
    case RENDER_MODE:
      if (event == EVENT_PRESS) {
        switch (key) {
          ...
          case 'l': case 'L':
          case ';': case '\'':
          case 'g': case 'G':  // Toggle ambient GI
          case 'x': case 'X':  // Toggle cache visualization
          case 'o': case 'O':  // Switch cache type
            renderer->stop();
