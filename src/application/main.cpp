...
void usage(const char *binaryName) {
  ...
  printf("  -g               Enable ambient GI caching\n");
  printf("  -V               Visualize ambient cache points in final render\n");
...
#define SETTINGSFILE_PATH "settings.txt"
    ...
    while ((opt = getopt(argc, argv, "s:l:t:m:o:e:h:H:f:r:c:b:d:a:p:gV")) !=
           -1) { // for each option...
      switch (opt) {
      case 'f':
        write_to_file = true;
      ...
      case 'H':
        ...
        break;
      case 'g':
        config.pathtracer_use_ambient_gi = true;
        break;
      case 'V':
        config.pathtracer_visualize_cache = true;
        break;
      default:
        usage(argv[0]);
        return 1;
