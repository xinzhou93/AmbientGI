...
Ray Camera::generate_ray(double x, double y) const {
	...
	// (x,y) right now is in the image space, it is normalized
	// 1. covert degrees to radians
	double hFov_rad = radians(hFov);
	double vFov_rad = radians(vFov);
	// 2. convert (x, y) to camera space
	double width_sensor = 2 * tan(0.5 * hFov_rad);
	double height_sensor = 2 * tan(0.5 * vFov_rad);

	double x_sensor = (x - 0.5) * width_sensor;
	double y_sensor = (y - 0.5) * height_sensor;

	// Point on sensor plane in camera space
	Vector3D camera_origin(0, 0, 0);
	Vector3D sensor_point = Vector3D(x_sensor, y_sensor, -1) - camera_origin;
	// Transform to world space
	Vector3D ray_dir = c2w * sensor_point;
	// Ray direction from camera position to world point
		// Camera coordinate system vectors
	//Vector3D w = c2w[2];  // view direction
	//Vector3D u = c2w[0];  // right direction
	//Vector3D v = c2w[1];  // up direction

	//// Build ray direction manually
	//Vector3D ray_dir = x_sensor * u + y_sensor * v + (-1) * w;
	ray_dir.normalize();
	//std::cout << "Camera generating ray: nClip=" << nClip << ", fClip=" << fClip << std::endl;
	Ray ray(pos, ray_dir, fClip);
	ray.min_t = nClip;
	return ray;
}
} // namespace CGL
