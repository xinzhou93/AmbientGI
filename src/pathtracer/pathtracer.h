...
#include "pathtracer/ambient_cache.h"
#include "pathtracer/octree_ambient_cache.h"
...
namespace CGL {
    ...
    public:
        ...
        Vector3D compute_ambient_lighting(const Ray& r, const SceneObjects::Intersection& isect);
        Vector3D compute_ambient_lighting_cached(const Ray& r, const SceneObjects::Intersection& isect);
        bool intersect_cache_visualization(const Ray& r, double& t, Vector3D& color);

        Vector3D at_least_one_bounce_radiance_helper(const Ray& r, const SceneObjects::Intersection& isect, int depth);
        Vector3D compute_ambient_lighting_helper(const Ray& r, const SceneObjects::Intersection& isect, int depth);
        ...
        bool use_ambient_gi; ///< true if using ambient GI caching instead of Monte Carlo
        bool visualize_cache; ///< true if visualizing cache points as colored spheres
        bool use_octree_cache; ///< true if using octree cache, false for simple grid cache

        // Components //

        BVHAccel* bvh;                 ///< BVH accelerator aggregate
        EnvironmentLight* envLight;    ///< environment map
        SimpleAmbientCache* ambient_cache; ///< simple grid-based ambient GI cache
        OctreeAmbientCache* octree_cache; ///< octree-based ambient GI cache with gradients
