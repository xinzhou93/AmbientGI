...
RaytracedRenderer::RaytracedRenderer(size_t ns_aa,
  ...
  pt->use_ambient_gi = use_ambient_gi;                      // Whether to use ambient GI caching
  pt->visualize_cache = visualize_cache;                    // Enable cache visualization if requested
...
void RaytracedRenderer::start_raytracing() {
  ...
  // Initialize octree cache if using ambient GI with octree
  if (pt->use_ambient_gi && pt->use_octree_cache) {
    pt->init_octree_cache();
  }
...
void RaytracedRenderer::key_press(int key) {
  ...
  case 'g': case 'G':
    pt->use_ambient_gi = !pt->use_ambient_gi;
    fprintf(stdout, "[PathTracer] Ambient GI caching %s\n", pt->use_ambient_gi ? "enabled" : "disabled");
    fflush(stdout);  // Force console output
    if (pt->use_ambient_gi && pt->use_octree_cache && !pt->octree_cache) {
      pt->init_octree_cache();
    }
    break;
  case 'x': case 'X':
    pt->visualize_cache = !pt->visualize_cache;
    fprintf(stdout, "[PathTracer] Cache visualization %s\n", pt->visualize_cache ? "enabled" : "disabled");
    fflush(stdout);  // Force console output
    break;
  case 'o': case 'O':
    if (pt->use_ambient_gi) {
      pt->use_octree_cache = !pt->use_octree_cache;
      fprintf(stdout, "[PathTracer] Using %s cache\n", pt->use_octree_cache ? "octree" : "grid-based");
      if (pt->use_octree_cache && !pt->octree_cache) {
        pt->init_octree_cache();
      }
    }
    break;
